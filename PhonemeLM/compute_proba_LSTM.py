...
def main(argv):
    ...
    lengths = dict()
    for file_id, pathQuantizedUnit in enumerate(args.pathQuantizedUnits.split(',')):
        with open(pathQuantizedUnit, 'r') as f:
            for line in f:
                file_name, file_seq = line.strip().split("\t")
                # Convert sequence to the desired input form
                file_seq = file_seq.replace(",", " ")
                # Add to lists
                if not file_name in input_file_dict:
                    input_file_dict[file_name] = []
                    lengths[file_name] = len(file_seq.split())
                if len(file_seq.split()) != lengths[file_name]:
                    print(f'Warning: {file_name}, {len(file_seq.split())} != {lengths[file_name]}')
                    lengths[file_name] = min(len(file_seq.split()), lengths[file_name])
                input_file_dict[file_name].append(file_seq)
                    
    input_file_names = sorted(input_file_dict)
    intput_file_seqs = [[' '.join(seq.split()[:lengths[k]]) for seq in input_file_dict[k]] for k in input_file_names]
    ...
    assert exists(join(pathData, "0/dict.txt")), \
    ...
    _ = compute_proba_LSTM(
                        ...
                        save_to=args.pathOutputFile, file_names=input_file_names, debug=args.debug)
