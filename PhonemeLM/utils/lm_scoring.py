...
def compute_proba_LSTM(
        ...
        save_to=None, file_names=None, debug=False):
    """
    ...
    try:  # In case of any errors, release GPU memory
        ...
        def compute_proba_batch(sequences):
            ...
            for sequence_list in sequences:
                input_sequence = []
                for sequence, source_dictionary in zip(sequence_list, task.source_dictionaries):
                    pad_idx = source_dictionary.pad()
                    # Convert from string to list of units
                    sentence_tokens = source_dictionary.encode_line("<s> " + sequence, append_eos=True, add_if_not_exist=False).long()

                    if print_tokens:
                        print("|".join([source_dictionary.symbols[tok] for tok in sentence_tokens]))
                    input_sequence.append(sentence_tokens)
                    
                # Update
                input_sequences.append(torch.stack(input_sequence, dim=-1))
                input_lengths.append(len(sentence_tokens))

            # Compute output & probabilities
            sentences_inputs = torch.nn.utils.rnn.pad_sequence(input_sequences, batch_first = True, padding_value = pad_idx)
            if gpu:
                sentences_inputs = sentences_inputs.cuda()
            outputs_ts = model(sentences_inputs, input_lengths)
            outputs_ts = [output_ts.softmax(dim=-1) for output_ts in outputs_ts]

            # Compute scores
            logproba_list = []
            for j, (sequence_list, sentence_inputs) in enumerate(zip(sequences, sentences_inputs)):
                logproba=0.
                for sequence, output_ts, sentence_input in zip(sequence_list, outputs_ts, sentence_inputs.t()):
                    for i, ch_idx in enumerate(sentence_input[1:]):
