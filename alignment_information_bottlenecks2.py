...
class AlignmentInfoBottleneck:
  ...
  def initialize(self, init_method='diagonal'):
    ...
    self.P_ZX_k = np.zeros((self.K_z, self.K_x+1))
    ...
    P_X_X_k = self.P_X[:, np.newaxis] * self.P_XX_k
    ...
    self.P_ZX_k = self.P_XZ.T @ P_X_X_k
    self.P_ZX_k /= np.sum(self.P_ZX_k, axis=-1, keepdims=True) + EPS
  ...
  def compute_joint_prob(self, X, Y):
    ...
    P_XX_k = 1. / (self.K_x+1) * np.ones((self.K_x+1, self.K_x+1))
    for x, y in zip(X, Y):
  ...
  def fit(self, max_epochs=100, 
    ...
    with open('{}_P_XX_k_top5.txt'.format(prefix), 'w') as f:
      f.write('X\tX_{t+k}\tP(X_{t+k}|X_t)\n')
      for x in range(self.K_x):  
        x_ks = np.argsort(-self.P_XX_k[x])[:5]
        for x_k in x_ks:
          f.write('{}\t{}\t{:.3f}\n'.format(x, x_k, self.P_XX_k[x, x_k]))
    ...
    for epoch in range(max_epochs):
      ...
      for k in range(self.K_z):
        KL[:, k] = kl_divergence(self.P_XY * self.P_XYA[:, :, 1], self.P_ZY[k] * 1./2) / 2.
        KL[:, k] += kl_divergence(self.P_XX_k, self.P_ZX_k[k]) / 2.
      ...
      # Update predictors
      P_ZY = P_XZ.T @ (P_X_Y * P_XYA[:, :, 1])
      P_ZY /= np.sum(P_ZY, axis=-1, keepdims=-1) + EPS 
      
      P_ZX_k = self.P_XZ.T @ P_X_X_k
      P_ZX_k /= np.sum(P_ZX_k, axis=-1, keepdims=True) + EPS

      # Update variational priors
      P_Z = self.P_X @ P_XZ
      P_A = np.sum(P_X_Y[:, :, np.newaxis] * self.P_XYA, axis=(0, 1))
      ...
      if epoch > 0 and abs(loss - prev_loss) <= abs(tol * prev_loss):
        logging.info('I( X ; Y ) = {:.3f}, H( X ) = {:.3f}, H( Y ) = {:.3f}'.format(I_XY, H_X, H_Y))
        logging.info('I( Z ; X ) = {:.3f}, I( Z ; Y ) = {:.3f}, I(Z ; X_k) = {:.3f}, H( Z ) = {:.3f}'.format(I_ZX, I_ZY, I_ZX_k, H_Z))
  ...
  def bottleneck_objective(self, alpha, beta):
    ...
    I_ZX_k = kl_divergence(P_Z_X_k.flatten(), (self.P_Z[:, np.newaxis] * self.P_X[np.newaxis, :]).flatten())
    IB = H_Z - alpha * H_XZ - beta * (I_ZY + I_ZX_k) / 2. 
    ...
    if not os.path.exists(prefix+'_tradeoff_X.csv') or not os.path.exists(prefix+'_tradeoff_Y.csv'):
      for alpha in 10**np.linspace(-3, 0, 4):
        for beta in 10**np.linspace(0, 2, 10):
          ...
          data_tradeoff_X_dict[r'$I(Z;X_{t+k})$'].append(mutual_infos[2][-1])
...
if __name__ == '__main__':
  ...
  bottleneck = AlignmentInfoBottleneck(corpus_path, P_X_Y=P_X_Y, K_c=2, K_z=10 if args.dataset == 'synthetic_image_caption' else 49)
