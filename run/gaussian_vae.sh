...
stage=1
stop_stage=100
model_root=/ws/ifp-53_2/hasegawa/lwang114/spring2021/DiscoPhoneInfoBottleneck/VIB-pytorch/checkpoints/phone_discovery_librispeech_wav2vec2_vae/
...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
  cwd=$(pwd)
  cd ../VIB-pytorch
  CUDA_VISIBLE_DEVICES=$gpu_num python main_visual_evidence_ib.py configs/librispeech_wav2vec2_vae.json || exit 1; 
  cd $cwd 
fi
