...
class Dataset(torch.utils.data.Dataset):
  def __init__(
    ...
    self.max_feat_len = 200
    self.max_token_len = 40
  ...
  def __getitem__(self, idx):
    ...
    try:
      inputs = self.transforms(audio[:, begin:end])
    except:
      inputs = self.transforms(audio)[:, :, int(begin // 160):int(end // 160)]
    ...
    outputs = self.preprocessor.to_index(label)
    ...
    if fn in filenames and "children" in phrase:
        if len(phrase["children"]) > 0:
            example = {"audio": filename,
                       "text": " ".join([w["text"] for w in phrase["children"]]), # re.sub(r"[^\w]", "", phrase["text"].lower()),
...
if __name__ == "__main__":
    preproc = FlickrSegmentPreprocessor(num_features=80, data_path="/ws/ifp-53_2/hasegawa/lwang114/data/flickr30k/")
