...
from model import Davenet, DotProductClassAttender
...
class Solver(object):
  ...
  def train(self):
    ...
    while epoch < args.epoch:
        ...
        for i, batch in enumerate(train_loader): 
            ...
            word_label_onehot = (F.one_hot(word_label, n_visual_class) * word_mask).sum(-2) 

            optimizer.zero_grad()
            
            audio_output = audio_model(audio_input, input_mask)
            ...
            word_logit, attn_weights = attention_model(audio_output, input_mask_ds).unsqueeze(-1)

            if args.loss_type == 'binary_cross_entropy': 
              loss = criterion(word_logit, word_label_onehot)
            else:
              loss = criterion(word_logit,
                               word_label_onehot,
                               torch.ones(B, 
                                          device=device))
            loss.backward()
  ...
  def validate(self):
    ...
    gold_labels = [] 
    pred_labels = [] 
    readable_f = open(f'keyword_predictions_{epoch}.txt', 'w')
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            gold_labels.append(word_label_onehot.detach().cpu().numpy())
            pred_labels.append(pred_label_onehot.detach().cpu().numpy())
            ...
            for ex in range(B):
              ...
              pred_idxs = np.nonzero(pred_label_onehot[ex])[0].tolist()
              gold_idxs = np.nonzero(word_label_onehot[ex])[0].tolist()
              ...
              readable_f.write(f'{audio_id}\t{pred_word_names}\t{gold_word_names}\n') 
  
        gold_labels = np.concatenate(gold_labels)
        pred_labels = np.concatenate(pred_labels)
    readable_f.close()
    precision, recall, f1, _ = precision_recall_f1_support(average='macro')
...
def main(argv):
    ...
    config = ConfigFactor.parse_file(args.CONFIG)
