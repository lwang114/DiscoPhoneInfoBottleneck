...
from phone_model import UnigramPronunciator, HMMPronunciator, LinearPositionAligner
...
class Solver(object):
  ...
  def __init__(self, config):
    ...
    self.phone_net = cuda(HMMPronunciator(self.visual_words,
                                          self.phone_set,
                                          config=config,
    ...
    for e in range(self.epoch):
      ...
      for idx, (audios, phoneme_labels, word_labels,\
        ...
        if self.loss_type == 'binary_cross_entropy':
          loss = self.weight_phone_loss * self.criterion(cluster_probs,
        ...
        if self.max_normalize:
          word_phone_probs = word_phone_probs / (word_phone_probs.max(-1, keepdim=True)[0] + EPS)

        if self.loss_type == 'binary_cross_entropy':
          loss = loss + self.weight_word_loss * self.criterion(word_cluster_probs,
                                                               word_phone_probs * word_masks.sum(-1).view(-1, self.max_word_len, 1))
        else:
          loss = loss + self.weight_word_loss * self.criterion(word_cluster_probs,
                                                               word_phone_probs,
