  ...
  def train(self, save_embedding=False):
    ...
    for e in range(self.epoch):
      ...
      for idx, batch in enumerate(self.data_loader['train']):
        ...
        pm_phone_probs = self.audio_net.reverse_forward(word_labels.flatten(), ignore_index=self.ignore_index)
        # (batch size x max word num, max word len, n phone class)
        ...
        if self.loss_type == 'cross_entropy':
          ...
          phone_loss = self.criterion(phone_probs,
                                      pm_phone_probs,
                                      word_masks_2d)
          phone_loss = 0. * phone_loss
          ...
          if self.max_normalize:
            word_probs = word_probs / word_probs.max(-1, keepdim=True)[0]
          # (batch size x max word num, n word class)
          word_labels_onehot = F.one_hot(word_labels, 
                                         self.n_visual_class)
          word_labels_onehot = word_masks_1d.unsqueeze(-1) * word_labels_onehot
          word_loss = self.criterion(word_probs,
                                     word_labels_onehot, 
                                     torch.ones(audio_lens.size(0),
                                                device=x.device)) 
        ...
        if self.global_iter % 1000 == 0:
          ...
          print(f'Itr {self.global_iter:d}\tAvg Loss (Total Loss):{avg_loss:.2f} ({total_loss:.2f})\tAvg Phone Loss:{avg_phone_loss:.2f}\tAvg Word Loss:{avg_word_loss:.2f}')
