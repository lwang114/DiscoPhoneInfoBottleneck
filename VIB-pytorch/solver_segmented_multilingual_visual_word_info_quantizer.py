...
class Solver(object):
  ...
  def get_model_config(self, config):      
    self.audio_net = cuda(InfoQuantizer(in_channels=self.input_size,
                                        ...
                                        z_dim=self.n_visual_class+self.n_phone_class,
                                        ), self.cuda)
    self.phone_net = ClassAttender(input_dim=self.input_size,
                                   hidden_dim=self.K,
  ...
  def train(self, save_embedding=False):
    ...
    for e in range(self.epoch):
      ...
      for idx, batch in enumerate(self.data_loader['train']):
        ...
        phone_labels = cuda(phone_labels, self.cuda)
        binary_phone_labels = F.one_hot(phone_labels, self.n_phone_class).float()
        ...
        if self.audio_feature == "wav2vec2":
          ...
          audio_masks = (audio_masks.sum(-1) > 0).float()
          
        # (batch size, max segment num)
        if audio_masks.dim() == 3:
          segment_masks = torch.where(audio_masks.sum(-1) > 0).float()
        else:
        ...
        # (batch size, max segment num, n visual class + n phone class)
        out_logits, quantized, cluster_loss = self.audio_net(x, masks=audio_masks)
        word_logits = out_logits[:, :, :self.n_visual_class]
        phone_logits = out_logits[:, :, self.n_visual_class:]
        phone_logits, _ = self.phone_net(phone_logits, masks=segment_masks)
        phone_logits = torch.matmul(binary_phone_labels, phone_logits)
        phone_loss = F.cross_entropy(phone_logits,
                                     phone_labels,
        ...
        word_loss = F.cross_entropy(segment_word_logits,
                               word_labels,
                               ignore_index=self.ignore_index)
        loss = phone_loss + word_loss + cluster_loss # self.beta * (F.softmax(phone_logits, dim=-1) * F.log_softmax(phone_logits, dim=-1)).sum((-1, -2)).mean()
    ...
    with torch.no_grad():
      ...
      for b_idx, batch in enumerate(self.data_loader['test']):        
        ...
        word_loss = F.cross_entropy(segment_word_logits,
                               word_labels,
                               ignore_index=self.ignore_index)
        loss = phone_loss + word_loss + cluster_loss
        total_loss += loss.cpu().detach().numpy()
        total_step += 1.
