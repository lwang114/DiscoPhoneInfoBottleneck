...
class Solver(object):
  ...
  def get_model_config(self, config):
    if config.model_type == 'blstm':
      ...
      self.phone_net = nn.Linear(2*self.K, self.n_phone_class) 
      ...
      self.phone_net = nn.Linear(self.K, self.n_phone_class)
    else:
    ...
    for e in range(self.epoch):
      ...
      for idx, batch in enumerate(self.data_loader['train']):
        ...
        embedding = self.audio_net(x,
                                   masks=audio_masks,
                                   return_feat=True)
        # (batch size, max segment num, n phone class)
        phone_logits = self.phone_net(embedding) 
        word_loss = F.cross_entropy(word_logits,
                                    word_labels.flatten(),
                                    ignore_index=self.ignore_index)
        phone_loss = F.cross_entropy(phone_logits.view(-1, phone_logits.size(-1)),
                                     phone_labels.flatten(),
                                     ignore_index=self.ignore_index) # TODO
