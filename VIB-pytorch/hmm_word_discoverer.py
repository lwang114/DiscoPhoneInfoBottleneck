...
class HMMWordDiscoverer:
  def __init__(self, config):
    trainingCorpusFile = os.path.join(config["pronounce_model_train_file"])
    modelName = os.path.join(config['ckpt_dir'], 'hmm_word_discoverer')
  ...
  def initialize(self, fileName):
    ...
    j = 0;
    for s in f:
        s_dict = json.loads(s.rstrip('\n'))
        tTokenized = [s_dict['words'][w_idx]['text'] for w_idx in s_dict['visual_words']]
        tTokenized.insert(0, NULL)
        self.tCorpus.append(tTokenized)
        
        fTokenized = s_dict['pseudo_phones']
...
if __name__ == '__main__':
  ...
  model = HMMWordDiscoverer(args.config)
  model.trainUsingEM(20, writeModel=True)
  model.printAlignment(os.path.join(config['ckpt_dir'], 'alignment'))
