...
class Solver(object):
  ...
  def get_model_config(self, config):      
    ...
    self.use_same_cluster_group = config.get('use_same_cluster_group', True)
  ...
  def cluster(self,
      ...
      with torch.no_grad():
        ...
        for split in ['train']:
          ...
          for b_idx, batch in enumerate(self.data_loader[split]):
            ...
            for idx in range(audios.size(0)):
              ...
              for p in range(audio_lens[idx]):
                k = f'{word_labels[idx]}_{p}'
                if not k in group_dict:
                  group_dict[split][k] = []
                group_dict[split][k].append(len(X_dict[split])+p)
        ...
        groups = list(group_dict[split].values())
        begin_time = time.time()
        if self.use_same_cluster_group:
          self.clusterer.fit(X, same_cluster_grps=groups)
        else:
        ...
        if self.use_same_cluster_group:
          ys = self.clusterer.predict(X_dict['test'], same_cluster_grps=groups)
        else:
