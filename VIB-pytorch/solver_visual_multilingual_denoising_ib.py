...
class Solver(object):
  ...
  def test(self, save_embedding=False, out_prefix='predictions'):
    ...
    word_f.write('Image ID\tGold label\tPredicted label\tPredicted label from phones\n')
    ...
    with torch.no_grad():
      ...
      for b_idx, (audios, phoneme_labels, word_labels,\
        ...
        word_logits,\
        encoding,\
        embedding = self.audio_net(audios, masks=audio_masks,
                                   return_feat=True)
        ...
        phone_word_logits,\
        denoised_encodings,\
        _ = self.phone_net(phoneme_vectors,
                           temp=temp,
                           num_sample=self.num_sample,
                           return_feat=True)
        ...
        for idx in range(audios.size(0)):
          ...
          if word_lens[idx] > 0:
            ...
            pred_words_from_phone = phone_word_logits[idx].max(-1)[1]

            gold_words = gold_words.cpu().detach().numpy().tolist()
            pred_words = pred_words.cpu().detach().numpy().tolist()
            pred_words_from_phone = pred_words_from_phone.cpu().detach().numpy().tolist()
            gold_word_labels.append(gold_words)
            pred_word_labels.append(pred_words)
            pred_phone_word_labels.append(pred_words_from_phone)
            ...
            pred_word_names_from_phones.append(preprocessor.tokens_to_word_text(pred_words_from_phone)
            pred_word_str_from_phone = ','.join(pred_word_names_from_phone[-1])

            word_f.write(f'{audio_id}\t{gold_word_str}\t{pred_word_str}\t{pred_word_str_from_phone}\n')
          ...
          gold_phone_label_denoised = phoneme_labels_denoised[idx, :sent_lens[idx]]
          ...
          phone_readable_f.write(f'Utterance id: {audio_id}\n'
                                 ...
                                 f'Denoised transcript: {gold_phone_label_denoised}\n'
    ...
    dist, n_tokens = compute_edit_distance(pred_word_names_from_phone, gold_word_names)
    pwer = float(dist) / float(n_tokens)

    print('[TEST RESULT]')
    print('Epoch {}\tLoss: {:.4f}\tWord Acc.: {:.3f}\tPTER: {:.3f}\tPhone WER: {:.3f}'\
          .format(self.global_epoch, avg_loss, acc, pter, pwer))
    ...
    if self.history['acc'] < acc:
      ...
      self.history['pter'] = pter
      self.history['pwer'] = pwer
  ...
  def load_checkpoint(self, filename='best_acc.tar'):
    ...
    if file_path.is_file():
      ...
      self.phone_net.load_state_dict(checkpoint['model_states']['phone_net'])
