...
from criterion import MicroTokenF1Loss
...
class Solver(object):
  ...
  def __init__(self, config):
    ...
    self.audio_net = cuda(BLSTM(embedding_dim,
  ...
  def train_phone_net(self):
    file_path = self.ckpt_dir.joinpath('pron_dict')
    ...
    print('Finish training pronounciation model after {time.time()-begin_time}s')
  ...
  def train(self, save_embedding=False):
    ...
    criterion = MicroTokenF1Loss(beta=self.beta)
    for e in range(self.epoch):
      ...
      for idx, (audios, phoneme_labels, word_labels,
        ...
        cluster_probs = (cluster_logits - cluster_logits.max(-1, keepdim=True)[0]).exp() 
        word_cluster_probs = torch.matmul(word_masks, cluster_probs).view(-1, self.max_word_len, self.n_phone_class)

        word_phone_probs = self.phone_net(word_labels.view(-1, self.n_visual_class))
        word_phone_probs = word_phone_probs / word_phone_probs.max(0, keepdim=True)[0].unsqueeze(1).expand(-1, self.max_word_len, -1)
        ...
        for idx in range(audios.size(0)):
          ...
          if word_num[idx] > 0:
            ...
            for word_idx in range(word_num[idx]):
              ...
              pred_visual_phone_names = ','.join([str(p) for p in pred_visual_phone_label])
              visual_phone_f.write(f'{audio_id}_{word_idx:03d} {pred_visual_phone_names}\n')
