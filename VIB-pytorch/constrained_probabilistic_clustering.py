...
class ConstraintDIB:
  ...
  def fit(self, X,
          same_cluster_grps=None, # List of lists of example ids that are assigned to the same cluster
          max_iters=80,
          tol=1e-2):
    EPS = 1e-10
    ...
    for i in range(max_iters):
      ...
      if same_cluster_grps is not None:
        grp_assignments = [np.argmin(divs[grp].mean(0), axis=-1) for grp in same_cluster_grps]
        assign_dict = {
          idx:grp_assignments[g_idx] \
          for g_idx, grp in enumerate(same_cluster_grps) \
            for idx in grp
        }
        assignments = [assign_dict[idx] for idx in sorted(assign_dict)]
      else:
  ...
  def predict(self, X, same_cluster_grps=None):
