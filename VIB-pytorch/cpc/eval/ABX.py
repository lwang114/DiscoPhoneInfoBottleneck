...
def ABX(feature_function,
        ...
        max_size_group=30):
    """
    Args: See ABX/abx_iterators.py for more details
        feature_function (function): either a function or a callable object.
        ...
        seq_list (list): list of items (fileID, path) where fileID refers to
                         the file's ID as used in path_item_file, and path is
                         the actual path to the input audio sequence
        distance_mode (str): name of the distance function, 'euclidean' or 'cosine' 
        step_feature (int): number of feature samples per second  
        modes (list): 'within' for within-speaker ABX, 'across' for across-speaker ABX 
        seq_norm (boolean): 
        cuda (boolean): 
        max_x_across (int):
        max_size_group (int): 
